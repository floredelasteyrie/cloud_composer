...
table_ga = sc.newAPIHadoopRDD('com.google.cloud.hadoop.io.bigquery.JsonTextBigQueryInputFormat',
...
print(type(table_ga))
table_ga = table_ga.toDF(['ind', "state"])
table_ga.show(5) # Display 10 results.
...
list_col = df_sizmek.columns[:80] 
#list_col = list_col + df_sizmek.columns['fullVisitorId', 'visitId']
#list_col2 = df_sizmek.select([c for c in df_sizmek.columns if c in ['fullVisitorId','visitId']]).show(5)
#list_col = list_col1 + list_col2
#print(list_col)
df_sizmek = df_sizmek.select([coo for coo in df_sizmek.columns if coo in list_col])
print(df_sizmek.dtypes)



##### I.C Concatenate visit_id et full_visitor_id to get a unique visitor id :
from pyspark.sql import functions as sf 
df_sizmek = df_sizmek.withColumn('joined_column', 
                    sf.concat(sf.col('AgencyName'),sf.lit('_'), sf.col('DeviceType')))


df_sizmek.select("AgencyName","DeviceType","joined_column").show(5)
