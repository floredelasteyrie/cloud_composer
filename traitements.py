...
print("####### df_global")
df_global.show()
#print(df_matche.count()) # TODO chequer le nombre de matcher : 205
...
############KPI Campagne fete des mères ############

####### 1. Perimetre etude 

# FIltre sur fête des mères 
# df_global = df_global.filter(col("WinningCampaignName") == "L-FÌ»te des mÌ¬res Programmatique PBU-03052018")
# df_global.groupBy("WinningCampaignName").agg({"WinningCampaignName" : 'count'}).show() 

# Filtre sur WinningEventType : view et diplay 
# df_global = df_global.filter(col("WinningEventType")  "View")

######## 2. Calcul KPI 

##### A. SOCIAL 
# KPI Session : filtrer sur nb de vues >= 2 

# KPI Bounce rate 
df_bounce_rate = df_global.select(["bounce"])
...
# Sessions
# 1. Group by channel
# 2. Count the number of unique session id (donné par la concaténation de fullVisitorId et visitId)
# df_global.groupBy("channel").agg({"concat_visitId_fullVisitor" : 'count'}).show()


# Bounce rate
# 1. Group by channel
# 2. Count the number of unique session id (donné par la concaténation de fullVisitorId et visitId)
# Selectionner colonnes Bounce et concat : 

# 
df_bounce = df_global.groupBy("channel").agg(F.sum("bounce").alias("SUM_nb_bounce"),
    F.count("concat_visitId_fullVisitor").alias("COUNT_concat_visitId_fullVisitor"))
df_bounce.show()
df_bounce = df_bounce.withColumn("ratio", col("SUM_nb_bounce") / col("COUNT_concat_visitId_fullVisitor"))
df_bounce.show()


############## Renvoyer table sur BQ ############

######## Export de la table social : dataframe df_social
